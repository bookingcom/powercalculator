...
const { tTest, gTest, getRelativeImpactFromAbsolute, getCorrectedAlpha } = math
...
const executeTest = (idx, val, apr) => {
  ...
  const out = `idx=${idx},out=${
    err < TOLERANCE
  },val=${val},apr=${apr},err=${err}`
  return out
}
  ...
  beforeAll(() => {
    try {
      const content = readFileSync(
        resolve(join(__dirname, './dataset.ttest.csv')),
        {
          encoding: 'utf-8',
          flag: 'r',
        }
...
describe('G-test dataset (only two-sided, no standard deviation)', () => {
  ...
  beforeAll(() => {
    try {
      const content = readFileSync(
        resolve(join(__dirname, './dataset.gtest.csv')),
        {
          encoding: 'utf-8',
          flag: 'r',
        }
